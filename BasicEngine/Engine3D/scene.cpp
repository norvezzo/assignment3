...
#include <glm/gtc/matrix_inverse.hpp>
	...
	void Scene::MouseProccessing(int button, RubiksCube* rubi)
	{
		if(pickedShape == -1)
		{
			if(button == 1)
			{				
				MoveCamera(0, xTranslate, xrel * 0.01f);
				MoveCamera(0, yTranslate, -yrel * 0.01f);
			}
			else
			{
				rubi->rotateRubiksCube(glm::vec3(1, 0, 0), -yrel / 2.0f);
				rubi->rotateRubiksCube(glm::vec3(0, 1, 0), -xrel / 2.0f);
			}
		}
	}
